...
def make_script_entry_point (migrations_dir, migrations_table_name = 'migrations'):
	...
	def main ():
		...
		_apply_migrations(pending_migrations, migrations_dir, migrations_table_name, db_session, log)
...
def _apply_migrations (migrations, migrations_dir, migrations_table_name, db_session, log):
	if migrations:
		for migration_id in migrations:
			fpath = migrations_dir + '/%s.sql' % migration_id
